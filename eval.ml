...
let rec eval_exp ((e : rexp), (pos : int)) : int =
  ...
  | And (e1, e2) -> if eval_exp e1 <> 0 then eval_exp e2 else 0
  | Or (e1, e2) -> if eval_exp e1 <> 0 then 1 else eval_exp e2
...
let rec eval_stmt ((s : rstmt), (pos : int)) : unit =
  ...
  | Seq (s1, s2) ->
      eval_stmt s1;
      eval_stmt s2
  | If (e, s1, s2) -> if eval_exp e <> 0 then eval_stmt s1 else eval_stmt s2
  | While (e, s1) ->
      eval_stmt (If (e, (Seq (s1, (s, pos)), pos), (skip, pos)), pos)
  | For (e1, e2, e3, s1) ->
